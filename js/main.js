let Match = {
    ...
    randomize: ()=>{
      ...
      let offset = Math.ceil(12*Math.random())
      Match.jjsArray = Match.randomArray.concat(Match.randomArray2)
      Match.jjsArray.forEach((x,i)=>{document.querySelectorAll('section')[i].classList=`jj${x+offset} flip`})
      console.log(Match.jjsArray)
    },
    ...
    flip: (e)=>{
      ...
      Match.removeEvents()
        ...
        // e.target.classList.toggle(`${Match.jjsArray[e.target.id]}`)
    ...
    check: ()=>{
  if(Match.jjsArray[Match.choices[0]]!==Match.jjsArray[Match.choices[1]]){
    setTimeout(()=>{
